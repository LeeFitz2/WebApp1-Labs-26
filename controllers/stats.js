...
const stats =
{
    createView(request, response)
    {
        ...
        let highestNumSongs = Math.max(...playlists.map(playlist => playlist.songs.length));
        let mostSongs = playlists.filter(playlist => playlist.songs.length === highestNumSongs);
        let favSongs = mostSongs.map(item => item.title);
        ...
        const statistics = 
        {
            ...
            displayMostSongs: highestNumSongs,
            displayFavSongs: favSongs
        };
